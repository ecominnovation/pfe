...
abstract class Application_Model_ModelAbstract
{
     ...
     *
     * @see Application_Model_DbTable_TableAbstract::getPrimaryKeyName()
     ...
     *
     * @see Application_Model_Mapper_MapperAbstract::toArray()
     * @return array
     */
     ...
     *
     * @param Application_Model_Mapper_MapperAbstract $mapper
     * @return Application_Model_ModelAbstract
     */
     ...
     *
     * @see Application_Model_Mapper_MapperAbstract::save()
     ...
     *
     * @param Application_Model_ModelAbstract $model Object to be compared to
     ...
     */
/*    public function diff(Application_Model_ModelAbstract $model,
     $ignorePrimaryKey = true, $relations = false, $load = true
        ...
        if ($relations) {
        	...
        	foreach($all_relations as $key => $property) {
        		...
        		if (is_null($value)) {
        			continue;
        		} elseif ($value instanceof Application_Model_ModelAbstract)  {
        			...
        			foreach ($value as $val) {
        				if ($val instanceof Application_Model_ModelAbstract) {
        		...
        		if (method_exists($model, $method)) {
        			...
        			if (is_null($other_value)) {
        				$other_value = array();
        			} elseif ($other_value instanceof Application_Model_ModelAbstract) {
